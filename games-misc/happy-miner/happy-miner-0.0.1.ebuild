...
DESCRIPTION="A game about a miner in a top-down 2D view, the goal is to earn money by extracting as much ore as possible."

# Homepage, not used by Portage directly but handy for developer reference
HOMEPAGE="https://github.com/Vlad50-50/curly-doodle"
...
# a dependency.
RDEPEND="sys-devel/gcc media-libs/libsdl2 media-libs/sdl2-image"

# Build-time dependencies that need to be binary compatible with the system
# being built (CHOST). These include libraries that we link against.
# The below is valid if the same run-time depends are required to compile.
DEPEND="sys-devel/gcc media-libs/libsdl2 media-libs/sdl2-image"

# Build-time dependencies that are executed during the emerge process, and
# only need to be present in the native build system (CBUILD). Example:
BDEPEND="sys-devel/gcc media-libs/libsdl2 media-libs/sdl2-image"
...
src_compile() {
	g++ main.cpp $(pkg-config --cflags --libs sdl2 SDL2_image) -o happy-miner || die "Build failed"
}
